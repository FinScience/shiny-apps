...
##                           http://nierhoff.info                             ##
##                                                                            ##
##       Live version of this app: http://apps.nierhoff.info/digitaslbi       ##
##                                                                            ##
##   Github: https://github.com/mhnierhoff/shiny-apps/tree/master/digitaslbi  ##
##                                                                            ##
################# ~~~~~~~~~~~~~~~~~ ######## ~~~~~~~~~~~~~~~~~ #################
...
getDataset1 <- reactive({
        switch(input$tabOne,
               "UBS" = tosa[,2],
               "Credit Suisse" = tosa[,3],
               "Goldman Sachs" = tosa[,4],
               "Morgan Stanley" = tosa[,5],
               "JPMorgan Chase" = tosa[,6])
})
...
output$cboxPlot <- renderPlot({
        ...
        bpdf <- tos[2:6]
        colnames(bpdf) <- c("UBS", "Credit Suisse", 
...
output$summaryCaption3 <- renderText({
        paste("1st Quartile:", round(quantile(getDataset1(), probs=0.25), 
                                     digits = 2))
})

output$summaryCaption4 <- renderText({
        paste("3rd Quartile:", round(quantile(getDataset1(), probs=0.75), 
                                     digits = 2))
})
...
output$summaryCaption12 <- renderText({
        paste("Median Absolute Deviation:", round(mad(getDataset1()), 
                                                  digits = 2))
})
...
getDataset5 <- reactive({
        switch(input$tabFive,
               ...
               "Credit Suisse" = tos[,3],
               "Goldman Sachs" = tos[,4],
               "Morgan Stanley" = tos[,5],
               "JPMorgan Chase" = tos[,6])
