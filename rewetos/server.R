...
## Getting the data for the websites         
        ...
        getDataset <- reactive({
                switch(input$page,
                       "rewe.de" = alexaTimeonSite[,2],
                       "toom-baumarkt.de" = alexaTimeonSite[,3],
                       "bipa.at" = alexaTimeonSite[,4])
        })
...
## Caption creation for tabpanels          
        ...
        output$caption1 <- renderText({
                paste("The time on site per user of", input$page, "with", 
        ...
        output$caption3 <- renderText({
                paste("The time on site per user of", input$page, 
...
## Forecasting model plot creation                       
        ...
        plotInput <- function() {
                ...
                a <- seq(as.Date(aToS$Date, format = "%d.%m.%y")[1] + 1, 
                         by = "months", length = length(date) + 11)
                axis(1, at = as.numeric(a)/365.25 + 1970, 
